...
void CAN_Manager::write_motor(uint16_t canID, uint16_t motorID, int32_t value) {
...
uint16_t CAN_Manager::get_motor_attribute(uint16_t canID, uint16_t motorID, MotorAttribute valueType) {
...
void CAN_Manager::print_motor(uint16_t canID, uint16_t motorID, bool showFullHex) {
    ...
    uint16_t speed = combine_bytes(m_input[canID][motorID][2], m_input[canID][motorID][2 + 1]);
    uint16_t torque = combine_bytes(m_input[canID][motorID][4], m_input[canID][motorID][4 + 1]);
...
void CAN_Manager::print_can(uint16_t canID, bool showFullHex) {
    // for all motors, print that motor
    for (int i = 0; i < NUM_MOTORS; i++)
        print_motor(canID, i, showFullHex);
}

void CAN_Manager::print_output() {
  for (int i = 0; i < NUM_CANS; i++) {
    Serial.printf("CAN: %x\n", i);
