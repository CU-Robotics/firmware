...
EngChassisYawEstimator::EngChassisYawEstimator(Config config_data, SensorManager* sensor_manager, CANManager* _can) {
    buff_enc_yaw = sensor_manager->get_buff_encoder(0); // sensor object definitions
    can = _can;
    icm_imu = sensor_manager->get_icm_sensor(0);
    YAW_ENCODER_OFFSET = config_data.sensor_info[0][2];
...
void EngChassisYawEstimator::step_states(float output[STATE_LEN][3], float curr_state[STATE_LEN][3], int override) {
    float yaw_enc_angle = (buff_enc_yaw->get_angle()) - YAW_ENCODER_OFFSET;
    while (yaw_enc_angle >= PI)
        yaw_enc_angle -= 2 * PI;
    while (yaw_enc_angle <= -PI)
    ...
    yaw_axis_spherical[2] = acos(imu_yaw_axis_vector[2] / __magnitude(imu_yaw_axis_vector, 3)) - (1.92-1.57); // phi
...
EngineerArmEstimator::EngineerArmEstimator(CANManager* _can) {
    can = _can;
}
...
void EngineerArmEstimator::step_states(float output[STATE_LEN][3], float curr_state[STATE_LEN][3], int override) {
    ...
    // output[1][2] = 0;
    // output[2][0] = can->get_motor_state(CAN_2, 5).position - pitch2_encoder_offset;
    // output[2][1] = can->get_motor_state(CAN_2, 5).speed;
    // output[2][2] = 0;
    // output[3][0] = can->get_motor_state(CAN_2, 6).position - yaw2_encoder_offset;
    // output[3][1] = can->get_motor_state(CAN_2, 6).speed;
    // output[3][2] = 0;
