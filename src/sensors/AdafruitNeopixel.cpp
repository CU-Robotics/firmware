#include "AdafruitNeopixel.hpp"
#include <time.h>

/*Adafruit_NeoPixel::~Adafruit_NeoPixel() {
    // 1. Free dynamically allocated pixel buffer memory
    if (pixels) {
        delete[] pixels; // Use delete[] for heap-allocated arrays
        pixels = nullptr; // Prevent dangling pointers
    }

    // 2. Set data pin back to input to prevent unintended signals
    if (pin >= 0) {
        pinMode(pin, INPUT);
    }
}*/
