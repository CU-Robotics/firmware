	...
	while(counter != 5){
		...
		while (Serial8.available() < (2*ET16S_PACKET_SIZE)) {
			if ((millis() - start_time) > timeout) { 
				logger.println(LogDestination::Serial, "Transmitter: DR16 by timeout"); 
		...
		if (raw_input[0] == 0x0f) {
			...
			logger.println(LogDestination::Serial, "Transmitter: DR16");
			return TransmitterType::DR16;
		}
	}
	
	logger.println(LogDestination::Serial, "Transmitter: ET16S");
