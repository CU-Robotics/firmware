...
void HelloRobot::run(){
    ...
    while (true) {
        ...
        if (transmitter_manager.is_hive_mode()) {
            ...
            last_feed = target_state_map->operator[](Cfg::StateName::Feeder).get_position();            
        }
        ...
        if ((feed - estimated_state_map->operator[](Cfg::StateName::Feeder).get_position() > 2 && transmitter_manager.is_teensy_mode()) ||
            (target_state_map->operator[](Cfg::StateName::Feeder).get_position() - estimated_state_map->operator[](Cfg::StateName::Feeder).get_position() > 2 &&
             transmitter_manager.is_hive_mode())) {
            Serial.printf("Feeder is lowkey jammed. current ball count: %f, feed: %f, hive target: %f\n",
						  estimated_state_map->operator[](Cfg::StateName::Feeder).get_position(), feed, target_state_map->operator[](Cfg::StateName::Feeder).get_position());
            feed = estimated_state_map->operator[](Cfg::StateName::Feeder).get_position() + 1;
        ...
        *reference_map = governor->step_reference_map(*target_state_map);
        ...
        if (not_safety_mode) {
            ...
            can.issue_safety_mode();
            governor->set_position_reference(Cfg::StateName::Feeder, estimated_state_map->operator[](Cfg::StateName::Feeder).get_position());

            feed = (fmod(fmod(estimated_state_map->operator[](Cfg::StateName::Feeder).get_position(), 1) + 1, 1) > 0.2)
				? (int)floor(estimated_state_map->operator[](Cfg::StateName::Feeder).get_position()) + 1
				: (int)floor(estimated_state_map->operator[](Cfg::StateName::Feeder).get_position()); // reset feed to the current state
